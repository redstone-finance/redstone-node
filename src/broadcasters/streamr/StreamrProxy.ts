...
// TODO: I've commented out this module in order to test if
// the streamr dependency broke redstone nodes performance
...
export class StreamrProxy {
  ...
  async tryCreateStream(name: string): Promise<string> {
    logger.info("tryCreateStream skipped");
    return "";
  ...
  public async publishToStreamByName(data: any, streamName: string) {
    ...
    logger.info("publishToStreamByName skipped");
  }
  ...
  private async createStream(id: string): Promise<string> {
    ...
    // return stream.id;
    logger.info("createStream skipped");
    return "";
  }
  ...
  private async doesStreamExist(streamId: string): Promise<boolean> {
    ...
    //   }
    // }
    logger.info("doesStreamExist skipped");
    return true;
  }
